...
// Step 2: User submits their Reletter API key → complete the OAuth flow
app.post("/submit-api-key", async (c) => {
  try {
    ...
    const quotaResp = await fetch(
      ...
      {
        headers: { "x-reletter-api-key": apiKey.trim() },
      },
    );
    if (!quotaResp.ok) {
...
<head>
	...
	<title>Reletter MCP - Enter API Key</title>
	<style>
		body {
		...
		input[type="text"]:focus {
			outline: none;
			border-color: #1791ff;
			box-shadow: 0 0 0 3px rgba(23,145,255,0.1);
		}
		button {
			width: 100%;
			padding: 0.75rem;
...
<body>
	<div class="container">
		<div class="card">
			<h1>Reletter MCP Server</h1>
			<p>Enter your Reletter API key to connect. You can find it on your <a href="https://reletter.com/account/settings">account page</a>.</p>
			${error ? `<div class="error">${error}</div>` : ""}
			<form method="post" action="/submit-api-key">
				<input type="hidden" name="state" value="${stateToken}">
				<label for="api_key">API Key</label>
				<input type="text" id="api_key" name="api_key" placeholder="Paste your Reletter API key" required autocomplete="off">
...
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Authorize | Reletter MCP</title>
	<style>
		body {
			font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
			line-height: 1.6;
		...
		.btn-primary { background: #1791ff; color: white; }
...
<body>
	<div class="container">
		<div class="card">
			<h1>Reletter MCP Server</h1>
			<p><strong>${clientName}</strong> is requesting access to your <a href="https://reletter.com/developers">Reletter API</a> subscription.</p>
